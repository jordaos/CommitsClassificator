...
_ = text_clf.fit(model_train.data, model_train.target)
...
for index, (doc, category) in enumerate(zip(docs_new, predicted)):
    ...
    shutil.copy(("contents/release_1.1.0/%s" % (evaluate[index][0])),
                ("contents/SGDC/release_1.1.0/%s/%s" % (model_train.target_names[category], evaluate[index][0])))


print('Usando SGDClassifier, a precisão é de %d%%' % (np.mean(predicted == twenty_test.target) * 100))
